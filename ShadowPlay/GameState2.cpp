...
#define LEVEL_HEIGHT 3 * TIER_HEIGHT
...
/********************************** CONSTRUCTOR FOR LEVEL 2, CHECKS IF LOAD GAME OR NOT & INITIALIZES ALL OBJECTS *************************************/
GameState2::GameState2(StateManager& state_manager, bool loadFromFile) : AbstractState(state_manager, loadFromFile),
player_(sf::Vector2f(25, 885), sf::Vector2f(120, 100), sf::Color::Transparent),
exit_(sf::Vector2f(1150, 160), sf::Vector2f(64, 100), sf::Color::Black),
key1_(sf::Vector2f(100, 0), sf::Vector2f(46, 100), sf::Color::Blue),
shadow_(sf::Vector2f(0, 0), sf::Vector2f(120, 100), sf::Color::Transparent),
switch1_(sf::Vector2f(1000, 885), sf::Vector2f(43.2, 100), sf::Color::Yellow, 0),
switch2_(sf::Vector2f(1300, 885), sf::Vector2f(43.2, 100), sf::Color::Yellow, 0),
destroyable_wall1_(sf::Vector2f(400, 885), sf::Vector2f(50, TIER_HEIGHT - FLOOR_OFFSET * 3), sf::Color::Transparent),
destroyable_wall2_(sf::Vector2f(450, 885), sf::Vector2f(50, TIER_HEIGHT - FLOOR_OFFSET * 3), sf::Color::Transparent),
undestroyable_wall1_(sf::Vector2f(1000, 885), sf::Vector2f(50, TIER_HEIGHT - FLOOR_OFFSET * 3), sf::Color::Transparent)
{
	...
	else
	{
		...
		destroyable_wall1_.Shape().setPosition(destroyable_wall1_.Shape().getPosition().x, destroyable_wall1_.set_Ypos(LEVEL_HEIGHT, 0, TIER_HEIGHT, FLOOR_OFFSET));
		...
		undestroyable_wall1_.Shape().setPosition(undestroyable_wall1_.Shape().getPosition().x, undestroyable_wall1_.set_Ypos(LEVEL_HEIGHT, 1, TIER_HEIGHT, FLOOR_OFFSET));
		...
		exit_.Shape().setPosition(exit_.Shape().getPosition().x, exit_.set_Ypos(LEVEL_HEIGHT, 1, TIER_HEIGHT, FLOOR_OFFSET));
		exit_.spriteImage.setPosition(exit_.Shape().getPosition().x - exit_.Shape().getSize().x/2,
		...
		key1_.Shape().setPosition(100, key1_.set_Ypos(LEVEL_HEIGHT, 0, TIER_HEIGHT, FLOOR_OFFSET));
		...
		//-------------------------------------------SOUNDS FOR THE GAME----------------------------------------
		good_job_ = BASS_SampleLoad(FALSE, "Sounds/keepgoing.wav", 0, 0, 1, NULL);
		main_channel_ = BASS_SampleGetChannel(good_job_, FALSE);
		BASS_ChannelSetAttribute(main_channel_, BASS_ATTRIB_VOL, 0.4f);
...
void GameState2::create_lights()// Creating the lighs done in here
{
	short int light1_x = 380; short int light1_y = 830; // x = 380
	...
	lights_image_[0]->Shape().setPosition(lights_image_[0]->Shape().getPosition().x, lights_image_[0]->set_Ypos(LEVEL_HEIGHT, 1, TIER_HEIGHT, FLOOR_OFFSET));
	...
	short int light3_x = 240; short int light3_y = 510;
	...
	lights_image_[2]->Shape().setPosition(lights_image_[2]->Shape().getPosition().x, lights_image_[2]->set_Ypos(LEVEL_HEIGHT, 2, TIER_HEIGHT, FLOOR_OFFSET));
	lights_image_[2]->set_In_Range(false);

	short int light4_x = 1300; short int light4_y = 510;
	...
	short int light5_x = 1450; short int light5_y = 610;
...
void GameState2::create_guards()// creation of guards done in here
{
	guards_.emplace_back(std::move(std::make_unique<Guard>(sf::Vector2f(1450, 560),
		sf::Vector2f(50, 100), sf::Color::Red)));
	guards_[0]->Shape().setPosition(guards_[0]->Shape().getPosition().x,
		guards_[0]->set_Ypos(LEVEL_HEIGHT, 2, TIER_HEIGHT, FLOOR_OFFSET));

	guards_.emplace_back(std::move(std::make_unique<Guard>(sf::Vector2f(1554, 100),
		sf::Vector2f(50, 100), sf::Color::Yellow)));
	guards_[1]->Shape().setPosition(guards_[1]->Shape().getPosition().x,
		guards_[1]->set_Ypos(LEVEL_HEIGHT, 0, TIER_HEIGHT, FLOOR_OFFSET));

}
void GameState2::create_walls()
{

}
GameState2::~GameState2(){} // Destructor
void GameState2::pause()// pause the State
{
	set_paused(true);
}
void GameState2::resume()// resume the state
{
	set_paused(false);
}

void GameState2::handle_input(const sf::Time& delta)// Handle the input for the State
{
...
void GameState2::savePlayerInfo(const char* filename)// Saving the player information to file
{
...
void GameState2::LoadGame(Player& player, Guard& guards, Light& lights)// Loading game method
{
...
void GameState2::readPlayerInfo(const char* filename)// reading in all of the saved information
{
...
void GameState2::CheckOtherLights(Light& light)//method for checking a light against all of the other lights in the game
{
	auto& inputManager = state_manager_.get_input_manager();
...
void GameState2::in_range()// method for checking if you're in range of another light
{
	for (auto& light : lights_image_)
	{
...
void GameState2::check_collisions()// checking the collisions in the game
{
	for (auto& light : lights_image_)
	{
	...
	if (guards_[0]->Shape().getPosition().x <= destroyable_wall2_.Shape().getPosition().x + destroyable_wall2_.Shape().getSize().x / 2)
	{
		guards_[0]->Shape().setPosition(destroyable_wall2_.Shape().getPosition().x + destroyable_wall2_.Shape().getSize().x / 2, guards_[0]->Shape().getPosition().y);
		guards_[0]->turnAround();
		guards_[0]->turnRight;

	}
}

void GameState2::update(const sf::Time& delta)// updating the game
{
	...
	for (auto& guard : guards_)
	{
		...
		if (guard->goto_game_over() == true)
			game_over_ = true;
		//TURN AROUND 
		if (guards_[0]->Shape().getPosition().x - guards_[0]->Shape().getSize().x / 2 < destroyable_wall2_.Shape().getPosition().x + destroyable_wall2_.Shape().getSize().x / 2 && switch2_.has_Pulled() == false)
		{
			//guards_[0]->Shape().setPosition(guards_[0]->Shape().getPosition().x + 10, guards_[0]->Shape().getPosition().y);
			guards_[0]->turnRight = true;
			guards_[0]->turnAround();
		}

		//TURN AROUND
		if (guards_[1]->Shape().getPosition().x - guards_[1]->Shape().getSize().x / 2 < destroyable_wall1_.Shape().getPosition().x + destroyable_wall1_.Shape().getSize().x / 2 && switch1_.has_Pulled() == false)
		{
...
void GameState2::render() // Drawing all of the Game Level
{
...
void GameState2::teleport_player(Light& light, int teleport_id, int floor_no)// Method for teleporting the player
{
...
void GameState2::handle_state_changes()// change of states handled in here
{
	auto& state_current = state_manager_;
	if (game_over_ == true)
	{
		...
		{
		cleanUp();
		state_current.change_state(state_manager_.state_ptr<GameState3>(false));
		}
		else
			if (change_MM == true)
...
void GameState2::cleanUp()// cleanup of the game
{
	lights_image_.clear();
	guards_.clear();
}