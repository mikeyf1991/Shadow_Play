...
Player::Player(sf::Vector2f position, sf::Vector2f size, sf::Color colour) // constructor for the player
{
	...
	//spritesheet set up
	if (!spriteTexture.loadFromFile("Player/Player Walk SpriteSheet-05.png"))
		std::cout << "Error couldnt load Player" << std::endl;
	spriteImage.setScale(0.2f, 0.2f);
