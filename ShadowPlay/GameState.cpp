...
/********************************** CONSTRUCTOR FOR LEVEL 1, CHECKS IF LOAD GAME OR NOT & INITIALIZES ALL OBJECTS *************************************/
...
key1_(sf::Vector2f(0, 0), sf::Vector2f(46, 100), sf::Color::Blue),
shadow_(sf::Vector2f(0, 0), sf::Vector2f(120, 100), sf::Color::Transparent),
switch1_(sf::Vector2f(860, 885), sf::Vector2f(43.2, 100), sf::Color::Yellow, 0),
destroyable_wall_(sf::Vector2f(400, 885), sf::Vector2f(50, TIER_HEIGHT - FLOOR_OFFSET * 3), sf::Color::Transparent),
undestroyable_wall_(sf::Vector2f(720, 885), sf::Vector2f(50, TIER_HEIGHT - FLOOR_OFFSET * 3), sf::Color::Transparent)
{
	setMusicState(false);
	...
	if (load_game_ == true)
	{
		...
		destroyable_wall_.Shape().setPosition(destroyable_wall_.Shape().getPosition().x, destroyable_wall_.set_Ypos(LEVEL_HEIGHT, 2, TIER_HEIGHT, FLOOR_OFFSET));
		destroyable_wall_.spriteImage.setPosition(destroyable_wall_.Shape().getPosition().x - destroyable_wall_.Shape().getSize().x / 2, destroyable_wall_.Shape().getPosition().y - destroyable_wall_.Shape().getSize().y / 2 - 5);

		undestroyable_wall_.Shape().setPosition(undestroyable_wall_.Shape().getPosition().x, undestroyable_wall_.set_Ypos(LEVEL_HEIGHT, 2, TIER_HEIGHT, FLOOR_OFFSET));
		undestroyable_wall_.spriteImage2.setPosition(undestroyable_wall_.Shape().getPosition().x - undestroyable_wall_.Shape().getSize().x / 2, undestroyable_wall_.Shape().getPosition().y - undestroyable_wall_.Shape().getSize().y / 2 - 5);
		...
		exit_.spriteImage.setPosition(exit_.Shape().getPosition().x - exit_.Shape().getSize().x *1.5, exit_.Shape().getPosition().y - exit_.Shape().getSize().y * 2 - FLOOR_OFFSET * 4);
		//==========================================================================================================================================================//
		create_lights();
		create_guards();
		...
		key1_.Shape().setPosition(580, key1_.set_Ypos(LEVEL_HEIGHT, 2, TIER_HEIGHT, FLOOR_OFFSET));
...
void GameState::create_lights() // Creating the lighs done in here
{
	short int light1_x = 650; short int light1_y = 1126;
	...
	short int light2_x = 370; short int light2_y = 807;
	...
	lights_image_[1]->Shape().setPosition(lights_image_[1]->Shape().getPosition().x, lights_image_[1]->set_Ypos(LEVEL_HEIGHT, 1, TIER_HEIGHT, FLOOR_OFFSET));
	lights_image_[1]->set_In_Range(false);

	short int light3_x = 920; short int light3_y = 807;
	...
	short int light4_x = 190; short int light4_y = 488;
	...
	lights_image_[3]->Shape().setPosition(lights_image_[3]->Shape().getPosition().x, lights_image_[3]->set_Ypos(LEVEL_HEIGHT, 2, TIER_HEIGHT, FLOOR_OFFSET));
	lights_image_[3]->set_In_Range(false);

	short int light5_x = 1170; short int light5_y = 488;
	...
	lights_image_[4]->Shape().setPosition(lights_image_[4]->Shape().getPosition().x, lights_image_[4]->set_Ypos(LEVEL_HEIGHT, 2, TIER_HEIGHT, FLOOR_OFFSET));
	lights_image_[4]->set_In_Range(false);

	short int light6_x = 480; short int light6_y = 169;
	...
	lights_image_[5]->Shape().setPosition(lights_image_[5]->Shape().getPosition().x, lights_image_[5]->set_Ypos(LEVEL_HEIGHT, 3, TIER_HEIGHT, FLOOR_OFFSET));
	lights_image_[5]->set_In_Range(false);

}
void GameState::create_guards() // creation of guards done in here
{
...
void GameState::create_walls()
{

}
GameState::~GameState(){} // Destructor
void GameState::pause() // pause the State
{
	set_paused(true);
} 
void GameState::resume() // resume the state
{
	set_paused(false);
}

void GameState::handle_input(const sf::Time& delta) // Handle the input for the State
{
...
void GameState::savePlayerInfo(const char* filename) // Saving the player information to file
{
...
void GameState::LoadGame(Player& player, Guard& guards, Light& lights) // Loading game method
{
...
void GameState::readPlayerInfo(const char* filename) // reading in all of the saved information
{
...
void GameState::CheckOtherLights(Light& light) //method for checking a light against all of the other lights in the game
{
	auto& inputManager = state_manager_.get_input_manager();
	...
	for (auto& otherLight : lights_image_)
	{
		...
		{
			if (otherLight->Shape().getPosition().x < light.Shape().getPosition().x)
				player_.walkleft = true;
			else
				player_.walkleft = false;
...
void GameState::in_range() // method for checking if you're in range of another light
{
	for (auto& light : lights_image_)
	{
...
void GameState::check_collisions() // checking the collisions in the game
{
	...
	if (player_.Shape().getGlobalBounds().intersects(destroyable_wall_.Shape().getGlobalBounds()) && switch1_.has_Pulled() == false)
	{
		if (player_.Shape().getPosition().x < destroyable_wall_.Shape().getPosition().x)
			player_.Shape().setPosition(destroyable_wall_.Shape().getPosition().x - destroyable_wall_.Shape().getSize().x / 2 - (player_.Shape().getSize().x / 2),
	...
	if (player_.Shape().getGlobalBounds().intersects(undestroyable_wall_.Shape().getGlobalBounds()))
	{
		if (player_.Shape().getPosition().x < undestroyable_wall_.Shape().getPosition().x)
			player_.Shape().setPosition(undestroyable_wall_.Shape().getPosition().x - undestroyable_wall_.Shape().getSize().x / 2 - (player_.Shape().getSize().x / 2),
			player_.Shape().getPosition().y);
		if (player_.Shape().getPosition().x > undestroyable_wall_.Shape().getPosition().x)
			player_.Shape().setPosition(undestroyable_wall_.Shape().getPosition().x + undestroyable_wall_.Shape().getSize().x / 2 + player_.Shape().getSize().x / 2,
			player_.Shape().getPosition().y);
...
void GameState::update(const sf::Time& delta) // updating the game
{
	...
	else if (shadow_.getVisibleState() == true)
	{
		shadow_.shadowImage.setPosition(shadow_.Shape().getPosition().x - (shadow_.Shape().getSize().x / 2),
			shadow_.Shape().getPosition().y + shadow_.Shape().getSize().y / 2 - 204);
	}

	if (shadow_.getCastState() == true)
	{
		shadow_.shadowCastImage.setPosition(player_.Shape().getPosition().x - (player_.Shape().getSize().x / 2),
			player_.Shape().getPosition().y + player_.Shape().getSize().y / 2);
	}
	player_.framecounter += player_.frameclock.restart().asMilliseconds();
}

void GameState::render() // Drawing all of the Game Level
{
	...
	{
		window.draw(destroyable_wall_.spriteImage);
	...
	window.draw(undestroyable_wall_.spriteImage2);
...
void GameState::teleport_player(Light& light, int teleport_id, int floor_no) // Method for teleporting the player
{
...
void GameState::handle_state_changes() // change of states handled in here
{
	auto& state_current = state_manager_;
	if (game_over_ == true)
	{
		...
		{
		cleanUp();
		state_current.change_state(state_manager_.state_ptr<GameState2>(false));
		}
		else
			if (change_MM == true)
			{
...
void GameState::cleanUp() // cleanup of the game
{
	lights_image_.clear();
	guards_.clear();
}
